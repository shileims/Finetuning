...
class OpenClipEmbedWrapper(Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.tokenize = lambda x: tokenizer(x, padding="max_length", return_tensors='pt')['input_ids']
        self.wrapped_embed = EmbeddingWrapper(transformer.get_input_embeddings(), tokenize=self.tokenize, superclass_string=superclass_string)
        self.text_transformer = transformer

    def get_text_enc(self, embeds):
        encoder_outputs = self.text_transformer.text_model.encoder(embeds)
        last_hidden_state = encoder_outputs[0]
        text_enc = self.text_transformer.text_model.final_layer_norm(last_hidden_state)
        return text_enc
    ...
    def forward(
        ...
        **kwargs
        ...
        text_enc = self.get_text_enc(text_embeds)

        superclass_text_enc = None

        if exists(superclass_text_embeds):
            superclass_text_enc = self.get_text_enc(superclass_text_embeds)
